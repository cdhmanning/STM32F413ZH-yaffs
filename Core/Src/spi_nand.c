...
const struct nand_command_def cmd_def_read_from_cache_quad = 	{ 0xeb, 2, 2, 0, 1, 1};
...
#if 0
#define cmd_def_program_load_x			cmd_def_program_load_1
#define cmd_def_program_load_random_x	cmd_def_program_load_random_1
#define cmd_def_read_from_cache_x		cmd_def_read_from_cache_1
#else
#define cmd_def_program_load_x			cmd_def_program_load_4
#define cmd_def_program_load_random_x	cmd_def_program_load_random_4
#define cmd_def_read_from_cache_x		cmd_def_read_from_cache_4
#endif
...
#define nand_qspi hqspi


static volatile uint32_t qspi_complete;
								...
								uint32_t data_size)
{
	...
	cd.AddressMode = cmd->quad_address ?
					  	  QSPI_ADDRESS_4_LINES :
						  QSPI_ADDRESS_1_LINE;

	if (cmd->n_addr == 0) {
		cd.AddressMode = QSPI_ADDRESS_NONE;
		cd.AddressSize = 0;
	} else if (cmd->n_addr == 1) {
		cd.AddressSize = QSPI_ADDRESS_8_BITS;
	} else if (cmd->n_addr == 2) {
		cd.AddressSize = QSPI_ADDRESS_16_BITS;
	} else {
		cd.AddressSize = QSPI_ADDRESS_24_BITS;
	}

	cd.AlternateByteMode = QSPI_ALTERNATE_BYTES_NONE;
	cd.AlternateBytes = 0;
	cd.AlternateBytesSize = 0;

	if (data_size == 0)
		cd.DataMode = QSPI_DATA_NONE;
	else if (cmd->quad_data)
		cd.DataMode = QSPI_DATA_4_LINES;
	else
		cd.DataMode = QSPI_DATA_1_LINE;

	cd.DdrHoldHalfCycle = 0;
	cd.DdrMode = 0;
	cd.DummyCycles = cmd->quad_address ? cmd->n_dummy * 2 : cmd->n_dummy * 8;

	cd.Instruction = cmd->opcode;
	cd.InstructionMode = QSPI_INSTRUCTION_1_LINE;
	...
	ret = HAL_QSPI_Command(&hqspi, &cd, 10);
	qspi_complete = 0;
	...
	if (data_size > 0) {
		if (1 || data_size < 10) {
			if (cmd->data_is_tx) {
				...
				ret = HAL_QSPI_Receive(&hqspi, data, 10);
			}
		} else {
			if (cmd->data_is_tx) {
				ret = HAL_QSPI_Transmit_DMA(&hqspi, data);
			} else {
				ret = HAL_QSPI_Receive_DMA(&hqspi, data);
			}
									 ...
									 const uint8_t *buffer, uint32_t buffer_size)
{
	return spi_nand_transaction(random_data ?
									&cmd_def_program_load_random_x :
									&cmd_def_program_load_x,
								offset,
					   ...
					   uint8_t *statusptr)
{
	...
	gpio_debug2(1);
	for(i = 0; i < n_ops; i++) {
		ret = spi_nand_cmd_read_from_cache_1(ops->offset, ops->buffer, ops->nbytes);
		ops++;
	}
	gpio_debug2(0);
...
void page_erase_test(void)
{
	...
	ret = spi_nand_read_page(page,
							 &op, 1,
						   	 &status);
	printf("read page before erase returned %d, status %02x\n",
			ret, status);
	...
	ret = spi_nand_read_page(page,
							 &op, 1,
						   	 &status);
	printf("read page after erase returned %d, status %02x\n",
			ret, status);
	print_buffer(buffer, sizeof(buffer));
}
...
void page_read_write_test(void)
{
	...
	printf("read page before write returned %d, status %02x\n",
			ret, status);
	print_buffer(buffer, sizeof(buffer));
